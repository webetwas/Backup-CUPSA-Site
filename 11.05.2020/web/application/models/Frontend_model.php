...
class Frontend_model extends CI_Model {
  ...
  public function __construct()
  {
	...
	public function increasePageVizite($page_id)
	{
		return $query = $this->db->query("UPDATE fe_pages SET vizite = vizite + 1 WHERE id_page = '" .$page_id. "'");
	}
  ...
  public function listPages($is_active) {
    $query = $this->db->query("
		SELECT {$this->tbl_pages}.atom_id, {$this->tbl_pages}.id_page, {$this->tbl_pages_structure}.is_active, {$this->tbl_pages}.title, {$this->tbl_pages}.title_ro, {$this->tbl_pages}.title_browser_ro FROM `" .$this->tbl_pages. "`
