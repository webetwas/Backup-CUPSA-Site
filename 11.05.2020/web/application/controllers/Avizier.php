...
class Avizier extends CI_Controller {
	...
	public function index()
	{
		...
		$page = $this->_Pagini->GetPage("avizier");//getpage
		...
		if($articole = $this->_Airdrop->get_airdrops_by_air_controller_and_node_slug('avizier', 'avizier'))
		{
			$viewdata["articole"] = $articole;
		}		
		/*Get breadcrumbs*/
		$dynamic_breadcrumbs = $this->_Item->msqlGetAll('breadcrumbs_aviziere', array("idpage" => $page->p->atom_id));
		$breacrumbs=array();
		$i=0;
		...
		$view = (object) [ 'html' => array(
			...
			5 => (object) ["viewhtml" => "pagini/avizier", "viewdata" => null],
