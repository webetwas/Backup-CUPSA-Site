...
class Media extends CI_Controller {
  ...
  public function __construct() {
    ...
    $this->load->model("Item_model", "_Item");// model/_Item
		...
		$this->load->model("Media_model", "_Media");
  ...
  public function index()
  {
		...
		if($items) {
			foreach($items as $keyitem => $item) {
				...
				if($airdrop = $this->_Nodes->airdrop_get_all_nodes_ids_assoc_by_atom($this->Air->air_id, $item->atom_id))
				{
					$viewdata["airdrop"][$keyitem] = $airdrop;
				}
			}
	...
	public function chosen_request_page_assoc()
	{
		...
		if(!is_null($atom_id) && !is_null($params))
		{
			foreach($params as $action => $proiect_id_assoc)
			{
				...
				{
					case 'selected':
						$id = $this->_Media->insert_breadcrumbs_assoc($atom_id, $proiect_id_assoc);
						break;
					case 'deselected':
						$this->_Media->remove_breadcrumbs_assoc($atom_id, $proiect_id_assoc);
						break;
				}
			}
		}
	...
	public function insert_breadcrumbs_assoc(){
		...
		if(!is_null($id_page) && !is_null($params))
		{
			...
			foreach($params as $action => $id_page_assoc)
			{
				...
				{
					case 'selected':					
						$id = $this->_Media->insert_breadcrumbs_assoc($id_page, $id_page_assoc);						
						break;
					case 'deselected':
						$this->_Media->remove_breadcrumbs_assoc($id_page, $id_page_assoc);
						break;
				}
			}
		}
      ...
      case UPDATE:
		if(isset($this->uriseg->id) && !is_null($this->uriseg->id)):
			...
			if($pages = $this->_Pagini->get_all_pages())
			{
				...
				if($pages_assoc = $this->_Media->get_all_assoc_breadcrumbs($this->uriseg->id))
				{
					$viewdata["page_pages"] = $pages_assoc;
				}
				if($breadcrumb_assoc = $this->_Media->get_all_assoc_breadcrumbs($this->uriseg->id))
				{
			...
			if(isset($_REQUEST["{$form_submititem}"])) {
				...
				$newDBPattern = (object) [ // Design Database Pattern
					...
					"design" => array(
						...
						"insert_date" => false,
						"item_date" => (!empty($this->input->post($viewdata["form"]->item->prefix . 'item_date')) ? date_format(date_create($this->input->post($viewdata["form"]->item->prefix . 'item_date')), 'Y-m-d') : null),
						"pdf_name" => false,
						"pdf_file" => false
					)
				]; 
