...
class Buletine_meteo extends CI_Controller {
  ...
  public function __construct() {
    ...
    $this->load->model("Item_model", "_Item");// model/_Item
		...
		//$this->load->model("Buletine_model", "_Buletine");
		...
		switch($this->uriseg->item)
		{
		  ...
		  case INSERT:
			...
			if(isset($_REQUEST["{$form_submititem}"])) {
						...
						if($insert) {
							$this->_Session->setFB_Pozitive(array("ref" => "Servicii", "text" => "Ai adaugat un buletin meteo!"));
		  ...
		  break;
			case DELETE:
				if(isset($this->uriseg->id) && !is_null($this->uriseg->id))
				{
					...
					$this->_Session->setFB_Pozitive(array("ref" => $this->Air->air_identplural, "text" => 'Ai sters un buletin meteo!'));
