...
class Airdrop_model extends CI_Model
{
	...
	//
	public function get_airdrops_and_images_by_air_controller_and_node_slug($air_controller, $node_slug,$order_by_id=null, $order_by_item_date=null)
	{
		...
		'
			SELECT
				...
				nd.slug AS node_slug,
				nd.denumire_ro AS node_denumire_ro,
				nd.i_titlu_ro AS node_titlu_ro,
				nd.i_titlu_en AS node_titlu_en,
				nd.i_subtitlu_ro AS node_subtitlu_ro,
				nd.i_subtitlu_en AS node_subtitlu_en,
				atoms.*,
		...
		if($order_by_id==true){
			...
			'
				...
				WHERE
					...
					nd.slug = "' .$node_slug. '"
				GROUP BY atoms.atom_id
				ORDER BY atoms.atom_id DESC
				;		
			';			
		}
		if($order_by_item_date==true){
			$sql =
			'
				SELECT
					ad.airdrop_id,
    ...
    public function getArticol($air_controller, $node_slug, $id_articol)
	{
        $air = $this->get_air_data_by_air_controller($air_controller);
        if(!$air)
        {
        ...
        '
            ...
            WHERE
                nd.is_active = 1 AND
                nd.slug = "' .$node_slug. '" AND
                ad.airdrop_id = "'.$id_articol.'"
            GROUP BY atoms.atom_id
            ORDER BY airdrop_order ASC
            ;
        ';
