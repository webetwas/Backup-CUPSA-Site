...
class Item_model extends CI_Model {
  ...
  public function msqlGet($table, $data)
	...
	public function msqlGetAndImages($table, $data)
	{
		$sql =
		'
			SELECT
				tbl.*,
				GROUP_CONCAT(DISTINCT pri.img ORDER BY pri.id ASC SEPARATOR ",") AS images,
				GROUP_CONCAT(DISTINCT ppi.img ORDER BY ppi.id ASC SEPARATOR ",") AS promo_images
			FROM
				' . $table . ' AS tbl LEFT JOIN
				proiecte_images AS pri ON tbl.atom_id = pri.id_item LEFT JOIN
				proiecte_promo_images AS ppi ON tbl.atom_id = ppi.id_item
			WHERE
				' . $data["c"] . ' = "' . $data["v"] . '"
			GROUP BY
				tbl.atom_id
   ...
   * @return [type]        [description]
   */
	public function msqlGetAll($table, $data = null, $order_by_id = null)
	{
		if(!is_null($data))
		foreach($data as $keyd => $d)
			$this->db->where($keyd, $d);

		if($order_by_id == true && ($table == "buletine_meteo" || $table == "apa_meteorica")) {
			$this->db->order_by("an", "desc");
		}
	...
	public function msqlGetAllByJoin($table, $table2, $atom_id)
	{
		...
		'
			...
			FROM
				...
				' . $table2 . ' AS t2 ON t1.proiect_id = t2.atom_id
			WHERE t1.atom_id = "' . $atom_id . '"
			;
		';
