...
function insert_breadcrumbs_assoc(params) {
	...
	$.ajax({
	url: '<?=base_url()?>proiecte/insert_breadcrumbs_assoc/',
	dataType: "JSON",
	type: 'POST',
...
function request_page_assoc(params) {
	$.ajax({
	url: '<?=base_url()?>proiecte/chosen_request_page_assoc/',
	dataType: "JSON",
	type: 'POST',
...
function upFileSuccess(id, img) {
  ...
  if(fileref == "poza_promo") imgpath = "<?=$imgpathitem;?>";
