<?php
class Imager_model_banners extends CI_Model {
	...
	public function upcreateimages($data)
	{
		...
		$path = '../web/public/upload/img/page/banners';
		foreach($imager as $key => $imager_size)
		{
			if($imager_size->size == "l")
			{
				...
				$path_by_size = $path;
				$temp_upload = upload_by_data($temp_upload_main, $path_by_size, $image . '.jpg', null, null, false);
			}
		}
		$data_insert = array('idpage' => $data->atom_id, 'img_ref' => 'banner1', 'img' => $image . '.jpg');
		$insert = $this->insert_image("fe_pages_banners", $data_insert);
