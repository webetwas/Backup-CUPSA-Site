...
class Item_model extends CI_Model {
  ...
  public function __construct()
  {
    ...
    public function checkIfUserExist($table, $data = null)
    {
        $results = array();
        if(!is_null($data))
        {
   ...
   */
    ...
    function updateItem($table, $array_data, $id_item_x)
    {
        $item = false;
        $item = $this->msqlGet($table, $id_item_x);
        if($item)
        {
            $this->db->where('id_item', $id_item_x['id_item']);
    ...
    function updateItemPdf($table, $array_data, $id_item_x)
    {
        $item = false;
        $item = $this->msqlGet($table, $id_item_x);
        ...
        if($item)
        {
            $this->db->where('atom_id', $id_item_x['atom_id']);
            $this->db->update($table, $array_data);
            // if($this->db->affected_rows() > 0) return true;
            return $item;
        }
