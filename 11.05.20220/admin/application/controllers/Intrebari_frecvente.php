...
class Intrebari_frecvente extends CI_Controller {
  ...
  public function __construct() {
    ...
    $this->load->model("Item_model", "_Item");// model/_Item
		...
		$this->load->model("Intrebari_frecvente_model", "_Intrebari");
  ...
  public function index()
  {
		...
		$viewdata["imgpathitem"] = SITE_URL.PATH_IMG_INTREBARI_FRECVENTE."m/";
	...
	public function chosen_request_page_assoc()
	{
		...
		if(!is_null($atom_id) && !is_null($params))
		{
			foreach($params as $action => $proiect_id_assoc)
			{
				...
				{
					case 'selected':
						$id = $this->_Intrebari->insert_breadcrumbs_assoc($atom_id, $proiect_id_assoc);
						break;
					case 'deselected':
						$this->_Intrebari->remove_breadcrumbs_assoc($atom_id, $proiect_id_assoc);
						break;
				}
			}
		}
	...
	public function insert_breadcrumbs_assoc(){
		...
		if(!is_null($id_page) && !is_null($params))
		{
			...
			foreach($params as $action => $id_page_assoc)
			{
				...
				{
					case 'selected':					
						$id = $this->_Intrebari->insert_breadcrumbs_assoc($id_page, $id_page_assoc);						
						break;
					case 'deselected':
						$this->_Intrebari->remove_breadcrumbs_assoc($id_page, $id_page_assoc);
						break;
				}
			}
		}
      ...
      case UPDATE:
		if(isset($this->uriseg->id) && !is_null($this->uriseg->id)):
			...
			if($pages = $this->_Pagini->get_all_pages())
			{
				...
				if($pages_assoc = $this->_Intrebari->get_all_assoc_breadcrumbs($this->uriseg->id))
				{
					$viewdata["page_pages"] = $pages_assoc;
				}
				if($breadcrumb_assoc = $this->_Intrebari->get_all_assoc_breadcrumbs($this->uriseg->id))
				{
				...
				case UPLOADIMG:
					...
					switch($this->input->post("filetarget"))
					{
						case UPIMGPOZA:
							...
							$locations->path = '../web/' .PATH_IMG_INTREBARI_FRECVENTE;
