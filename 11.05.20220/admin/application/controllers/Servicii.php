...
class Servicii extends CI_Controller {
  ...
  public function __construct() {
    ...
    $this->load->model("Item_model", "_Item");// model/_Item
        ...
        $this->load->model("Servicii_model", "_Servicii");
  ...
  public function index()
  {
        ...
        $viewdata = array("items" => null, "controller_fake" => $this->controller_fake, "uri" => null, "imgpathitem" => null , "item"=>null);
        $viewdata["uri"] = $this->uriseg;
        $viewdata["imgpathitem"] = SITE_URL.PATH_IMG_SERVICII."m/";
		...
		/*Breadcrumbs*/	 
		$item= $this->_Pagini->get_page_by_slug($this->Air->air_table);
		$item= $item[0];
		$viewdata["item"] = $item;
		...
		if($pages = $this->_Pagini->get_all_pages())
		{
			...
			if($pages_assoc = $this->_Servicii->get_all_assoc_breadcrumbs($item->atom_id))
			{
				$viewdata["page_pages"] = $pages_assoc;
			}
			if($breadcrumb_assoc = $this->_Servicii->get_all_assoc_breadcrumbs($item->atom_id))
			{
	...
	public function chosen_request_page_assoc()
	{
		...
		if(!is_null($atom_id) && !is_null($params))
		{
			foreach($params as $action => $proiect_id_assoc)
			{
				...
				{
					case 'selected':
						$id = $this->_Servicii->insert_breadcrumbs_assoc($atom_id, $proiect_id_assoc);
						break;
					case 'deselected':
						$this->_Servicii->remove_breadcrumbs_assoc($atom_id, $proiect_id_assoc);
						break;
				}
			}
		}
	...
	public function insert_breadcrumbs_assoc(){
		...
		if(!is_null($id_page) && !is_null($params))
		{
			...
			foreach($params as $action => $id_page_assoc)
			{
				...
				{
					case 'selected':					
						$id = $this->_Servicii->insert_breadcrumbs_assoc($id_page, $id_page_assoc);						
						break;
					case 'deselected':
						$this->_Servicii->remove_breadcrumbs_assoc($id_page, $id_page_assoc);
						break;
				}
			}
		}
        ...
        switch($this->uriseg->item)
    {
      case UPDATE:
        if(isset($this->uriseg->id) && !is_null($this->uriseg->id)):
			...
			{
				$viewdata["pages"] = $pages;
				if($pages_assoc = $this->_Servicii->get_all_assoc_breadcrumbs($this->uriseg->id))
				{
					$viewdata["page_pages"] = $pages_assoc;
				}
				if($breadcrumb_assoc = $this->_Servicii->get_all_assoc_breadcrumbs($this->uriseg->id))
				{
					$viewdata["breadcrumb_assoc"] = $breadcrumb_assoc;
				}
			}				
        ...
        switch($this->uriseg->ajax)
            {
                ...
                case UPLOADIMG:
                    ...
                    switch($this->input->post("filetarget"))
                    {
                        case UPIMGPOZA:
                            ...
                            $locations->path = '../web/' .PATH_IMG_SERVICII;
                ...
                case 'upload_pdf':
                    ...
                    {
                        case UPIMGPOZA:
                            $locations->table = 'servicii';
                            $locations->path = '../web/' .PATH_SERVICII_PDF;

                            $file_upload_pdf = process_upload_file($inpfile_pdf, $locations->path);

                            if($file_upload_pdf)
                            {
                                $res["status"] = 1;
                                $res["pdf"] = $file_upload_pdf;

                                $atom_id = array("atom_id" => trim(intval($this->uriseg->id)));
                                $update_data = array("pdf" => $file_upload_pdf);

                                $insertitem = $this->_Item->updateItemPdf($locations->table, $update_data, $atom_id);
                                if($insertitem) $res["id"] = $insertitem->atom_id;
                            }
