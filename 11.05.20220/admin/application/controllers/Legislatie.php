...
class Legislatie extends CI_Controller {
  ...
  /**
    ...
    public function __construct() {
		...
		$this->load->model("Legislatie_model", "_Legislatie");
  ...
  public function index()
  {
		...
		if($pages = $this->_Pagini->get_all_pages())
		{
			...
			if($pages_assoc = $this->_Legislatie->get_all_assoc_breadcrumbs($item->atom_id))
			{
				$viewdata["page_pages"] = $pages_assoc;
			}
			if($breadcrumb_assoc = $this->_Legislatie->get_all_assoc_breadcrumbs($item->atom_id))
			{
	...
	public function chosen_request_page_assoc()
	{
		...
		if(!is_null($atom_id) && !is_null($params))
		{
			foreach($params as $action => $proiect_id_assoc)
			{
				...
				{
					case 'selected':
						$id = $this->_Legislatie->insert_proiect_assoc($atom_id, $proiect_id_assoc);
						break;
					case 'deselected':
						$this->_Legislatie->remove_proiect_assoc($atom_id, $proiect_id_assoc);
						break;
				}
			}
		}
	...
	public function insert_breadcrumbs_assoc(){
		...
		if(!is_null($id_page) && !is_null($params))
		{
			...
			foreach($params as $action => $id_page_assoc)
			{
				...
				{
					case 'selected':					
						$id = $this->_Legislatie->insert_breadcrumbs_assoc($id_page, $id_page_assoc);						
						break;
					case 'deselected':
						$this->_Legislatie->remove_breadcrumbs_assoc($id_page, $id_page_assoc);
						break;
				}
			}
		}
