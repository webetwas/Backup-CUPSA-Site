...
class Stiri extends CI_Controller {
  ...
  public function __construct() {
    ...
    $this->load->model("Item_model", "_Item");// model/_Item
		...
		$this->load->model("Stiri_model", "_Stiri");
  ...
  public function index()
  {
	...
	public function chosen_request_page_assoc()
	{
		...
		if(!is_null($atom_id) && !is_null($params))
		{
			foreach($params as $action => $proiect_id_assoc)
			{
				...
				{
					case 'selected':
						$id = $this->_Stiri->insert_breadcrumbs_assoc($atom_id, $proiect_id_assoc);
						break;
					case 'deselected':
						$this->_Stiri->remove_breadcrumbs_assoc($atom_id, $proiect_id_assoc);
						break;
				}
			}
		}
	...
	public function insert_breadcrumbs_assoc(){
		...
		if(!is_null($id_page) && !is_null($params))
		{
			...
			foreach($params as $action => $id_page_assoc)
			{
				...
				{
					case 'selected':					
						$id = $this->_Stiri->insert_breadcrumbs_assoc($id_page, $id_page_assoc);						
						break;
					case 'deselected':
						$this->_Stiri->remove_breadcrumbs_assoc($id_page, $id_page_assoc);
						break;
				}
			}
		}
	...
	public function item()
	{
		...
		$viewdata = array("controller" => $this->controller, "nodes" => null, "controller_fake" => $this->controller_fake, "controller_ajax" => $this->controller_fake. "/ajax/", "item" => null, "item_links" => null, "air" => $this->Air, "uri" => null, "form" => (object) [], "imgpathitem" => null, "airdrop" => null, "imager" => null);
		$viewdata["uri"] = $this->uriseg;
		$viewdata["imgpathitem"] = SITE_URL.PATH_IMG_STIRI."m/";
      ...
      case UPDATE:
		...
		if($imager_return)
		{
			...
			if($pages = $this->_Pagini->get_all_pages())
			{
				...
				if($pages_assoc = $this->_Stiri->get_all_assoc_breadcrumbs($this->uriseg->id))
				{
					$viewdata["page_pages"] = $pages_assoc;
				}
				if($breadcrumb_assoc = $this->_Stiri->get_all_assoc_breadcrumbs($this->uriseg->id))
				{
